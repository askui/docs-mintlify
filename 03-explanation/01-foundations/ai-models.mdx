...
1. **Large Action Model** creates a plan with specific steps
2. **Query Models** interpret any unclear instructions
3. **Grounding Models** execute each individual action
4. **Large Action Model** checks results and continues or adjusts the plan

For example, with "Book a flight from Berlin to Rome":
1. LAM plans: open travel site → search flights → select options → book
2. Grounding model clicks on flight search
3. Query model interprets "Berlin" and "Rome" as departure/destination
4. Grounding model fills in the form fields
5. LAM monitors progress and handles any errors
