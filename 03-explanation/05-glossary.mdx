...
| **action**                              | A method in the AskUI Control Client API that describes an action to be taken against the operating system, e.g., `click()`, `type()`. See [AskUI SDK API Actions](/04-reference/01-agent-frameworks/02-python/02-vision-agent-api/agent).                         |
