...
agent.click(
    loc.Element("button").right_of(loc.Text("Username"))
)
```

#### Check Multi-Monitor Setup
```python
# Specify display for multi-monitor systems
agent.click("Submit", display=0)  # Primary display
agent.click("Submit", display=1)  # Secondary display
```
...
agent.click(
    loc.Element("button")
        .with_text("Submit")
        .below(loc.Text("Terms"))
        .within(loc.Element("form"))
)
```
...
<AccordionGroup>
  ...
  <Accordion title="Use Fuzzy Matching">
    ...
    agent.click(loc.Text().contains("Hel"))
    ```
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Use Positional Selectors">
    ...
    agent.click(
        loc.Element("textfield").right_of(loc.Element("icon"))
    )
    ```
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Use Relative Positioning">
    ...
    agent.click(
        loc.Element("textfield").right_of(loc.Text().contains("Name"))
    )
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Use Proximity Selection">
    ...
    agent.click(loc.Text("Johnson").near(loc.Text("Alice")))
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Wait for Full Render">
    ```python
    import time
    time.sleep(2)  # Give UI time to fully render
    ```
  </Accordion>
...
agent.click(loc.Text("Submit").within(form_area))
```
