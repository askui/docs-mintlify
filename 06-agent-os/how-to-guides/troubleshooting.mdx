...
The AgentOS Core Service and the Remote Device Controller are both spawned in the interactive user session. For troubleshooting, collect logs from **both** locations:

**Interactive user session** (Core Service & Remote Device Controller):

```
%userprofile%\.askui\Logs
```

**Service wrapper** (runs as SYSTEM):
