...
### 🏋️ Online-Training in AskUI includes:
...
- The first two `click()` commands don't count as asks, as they don't require screenshots to be taken.
- The third action, `ask('is the home site visible?')`, requires a screenshot to check the visibility of the element, so it counts as **one ask/inference call**.
