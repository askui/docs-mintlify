...
<Steps>
  ...
  <Step title="Create Your First Automation">
    ...
    with VisionAgent(
        ...
        agent.click("Search Amazon input field")
        ...
        agent.act("""
        You are testing amazon.com for functionality and have searched for nike shoes.
        Simulate an online shopper:
        Look for the first option which is below Results and click on the first picture from the left
        """)
        agent.wait(2)  # Wait for product page

        # Add to cart
        agent.click("Add to Cart")
        agent.wait(2)  # Wait for cart update
        ...
        cart_status = agent.get("Is the shoe in the cart and is a value calculated?")
    ...
    6. Generate an HTML report of the automation
  </Step>
  <Step title="Understanding the Code">
    ...
    3. **Complex Actions**
       ...
       """)
       ```
       - Uses AI to perform complex, multi-step actions
       - Provides context for better decision making

    4. **Information Extraction**
       ```python
       cart_status = agent.get("Is the shoe in the cart...")
       ```
