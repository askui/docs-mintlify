...
with VisionAgent() as agent:
    ...
    agent.click("email download")
    datetime = agent.get("Simulate: retrieve invoice number from pdf")

    agent.act("""
    You are a CRM expert and filling it details
    Simulate a online shopper:

     take the invoice and enter it into hubspot crm

    """)


    # Extract information from the screen
    datetime = agent.get("Simulate: What is the datetime at the top of the screen?")
    print(datetime)
