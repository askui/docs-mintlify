...
|  | **Description** | **Models** | **Example (Python)** |
| --- | --- | --- | --- |
| `exists()` | Checks if an element exists without failing the test | All | `element_exists = agent.exists('Login button')` |
| `notExists()` | Checks if an element does not exist | All | `element_absent = agent.notExists('Error message')` |
| `ask()` | Asks the model a question about the current screen | All | `response = agent.ask('What is the current page title?')` |
