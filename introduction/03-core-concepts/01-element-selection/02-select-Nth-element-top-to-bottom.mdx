...
it('Select Nth Element from top to bottom', async () => {
    ...
    async function clickOnTopNElement(customImage: string, index: number = 0){
      const elements = (await aui.get().customElement({customImage: '/Users/Desktop/Screenshot 2025-05-14 at 11.41.45.png'}).exec()).sort((element1, element2) => element1.bndbox.ymin - element2.bndbox.ymin)
